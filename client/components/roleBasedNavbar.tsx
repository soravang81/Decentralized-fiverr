        ...
        <Link href={href} passHref>
            <motion.div
                className={`relative overflow-hidden group px-4 py-2 rounded-md cursor-pointer ${
                    isActive ? 'text-purple-400' : 'text-purple-300 hover:text-pink-400'
                } transition-colors duration-300`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
            >
                <span className="relative z-10">{children}</span>
                <span className={`absolute inset-0 bg-purple-600 opacity-20 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300 rounded-md ${
