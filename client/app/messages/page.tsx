...
import { getBuyerSideClients } from "../actions/messaging/client"
...
export default function Component() {
  ...
  const getData = useCallback(async () => {
    try {
      if(session && status === "authenticated") {
        ...
        console.log(session)
        setSessionId(session.user.id)
        const fetchedClients: {active: Client[], inactive: Client[]} = await getBuyerSideClients()
  ...
  return (
    <div className="mt-20 xl:mx-20 lg:mx-16 md:mx-10 sm:mx-4 h-[calc(100vh-86px)] overflow-y-hidden flex flex-col bg-background border rounded-lg">
